...
export function buildEvidence({ item, finding = null, discrepancy = null, ledger = null, ej = null, tills = null, drawer = null, cft = null, cfg = DEFAULT_CFG }) {
  ...
  if (tills) {
    ...
    if (!fc && (verdict === "flip" || verdict === "suspect_flip") && !advFlip && otherReg) why.push({ kind: "flip_who_none", text: `No till check-ins are logged for reg ${reg} or reg ${otherReg} on ${date} (typical for self-checkouts, whose cash lives in the recycler), so the log cannot say who swapped them.` });
