 ...
 *   sourcePullMeta: object   — { startDate, endDate, pulledAt, marketNumber }
 *                              from the IndexedDB pull record, for the cover
 */
export function buildDocDefinition(state, options) {
  ...
  // Market comes from the pull, not from a module constant. Null for pulls
  // taken before the field existed, or when no home market is set — in which
  // case the report simply omits the market rather than asserting one.
  const marketLabel = sourcePullMeta.marketNumber
    ? `Market ${sourcePullMeta.marketNumber}`
    : null;
      ...
      // Top brand strip
      {
        columns: [
          ...
          { width: "*", margin: [12, 8, 0, 0], stack: [
            ...
            ...(marketLabel
              ? [{ text: marketLabel, color: C.blue, fontSize: 13, margin: [0, 2, 0, 0] }]
              : []),
          ]},
        ],
        margin: [0, 0, 0, 28],
  ...
  return {
    ...
    info: {
      title:    `Claims Disposition Report${marketLabel ? ` — ${marketLabel}` : ""}`,
    ...
    footer: (currentPage, pageCount) => ({
      margin: [50, 0, 50, 24],
      columns: [
        { text: `${marketLabel ? `${marketLabel} · ` : ""}Generated ${formatGeneratedAt(generatedAt)}`,
