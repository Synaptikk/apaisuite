...
**Last reviewed:** 2026-09-15 · **Suite version:** 1.0.7
...
| Register L/S Triage | `registerls` | alpha | Clears the APPRISS WorkView register long/short queue: joins each work item to the Power BI long/short grid (reuses `livedashboard`'s register engine for flip/bounceback matching), the Power BI Cash Recycler till log (who checked tills in/out, cash advances), the APPRISS Cash Research ledger, and EJ receipts for the register-day; sorts items into ready-to-close / needs-a-look with a plain-English why; "Fill in APPRISS" drives the WorkView disposition form (reason + text) and stops at Complete; flags the single cash transaction that explains an unmatched shortage; per-associate error ledger with coaching notes and one-CSV-per-cashier export. See `dev/REGISTER_LS_FINDINGS.md`. |
| BoB and Lisa | `boblisa` | alpha | Missed-item finder over the whole EJ store-day: same card `TOKEN`, second sale 1–15 min later, <5 items, ≥$3. Tabs: manned lanes 9–25 (cashier miss), unmanned registers (possible theft), unpaid training receipts. Training receipts (Money Center 92–94, Vision Center 98) matched by UPC; T2 at the Vision Center flagged. Fetches AND analyzes inside a fresh EJ tab (a store-day is ~10 MB; a frozen tab hangs executeScript). CCTV links by store + register + time (no transaction id; `lib/video.js`). See `dev/BOBLISA_FINDINGS.md`. |
| Safety Agent Dashboard | `safetyagent` | alpha | SafeIQ computer-vision hazard alerts for one store, all available dates: camera × disposition-tag pivot (sortable, heat-shaded no_hazard_found share, three reviewer families or all 12 tags), associate table with median ack / done minutes, hour-of-day chart, click-to-expand no_hazard_found events. Pulls straight from SafeIQ Studio's raw SQL endpoint (`lib/sql.js`); the `X-SafePass-Token` header is captured by the shell's webRequest filter and replayed from the SW, with a background SSO tab as fallback. `dev/safeiq-sql.mjs` runs ad-hoc SQL against the same endpoint over the debug Edge. |
...
| Accident Details | `accidents` | alpha | The Live Dashboard "Accident Evidence" card grown into a module. One pull: CAS static HTML (evidence reports + FY PNL charge tables, credits flagged as reversed charges) enriched per claim from Clearsight PROD via **read-only GETs** (quick search resolves Ref # → ClaimID + coverage, FormData gives description + Evidence Collection checklist, Supplemental Information gives customer/witness statements) composed into a plain-text "what happened" summary. Auto-SSO in a background tab when the Clearsight session is cold. PNL-only refs (incl. legacy C…/L… numbers) resolve on demand. See `CURRENT_TASKS.md` §5c. |
...
| Cost Inventory | `costinventory` | alpha | Monthly Fresh cost inventory worksheet, filled from four systems: beginning inventory (OneWalmart lookup tool, CSRF-guarded JSON API), sales + purchases (Ops Portal ITR tRPC, summed over the fourth-Tuesday window), plus a separate "Unfinalized Trailer totals" panel for last night's MP/FDD freight (CaseVisibility for which trailers and what type x GDP Connect BigQuery for what they cost) — reported beside the sheet, never added into it, so rows 8/9/10 always export as zero. Exports by patching the store's own xlsx — formatting and formulas intact. Only the Cost Inventory App count is typed by hand. See `dev/COST_INVENTORY_FINDINGS.md`. |
| ClosingList | `closinglist` | live | Closing-shift email draft from CaseVisibility + IVR call-offs |
| StockingPlan | `stockingplan` | live (0.3.2) | Overnight stocking plan. Freight from CaseVisibility at three levels — area roll-up, per department and D92/95-by-aisle with CV's own stocking-time estimate — computed in `main.html`'s MAIN world off `psn_apiJson` + `calc_*`/`config_*`, never by opening `casesByDept.html`/`casesByAisle.html` (`window.open` steals focus, and CV's `config_dept*` tables are `let`/`const`, invisible on `window`) — sized against the night's labour: Stock 2 + Overnight scheduled hours vs required freight hours, plus **tomorrow's** Stock 1 crew, because a plan written on date D covers Stock 2 on D, Overnight 22:00 D→07:00 D+1, and Stock 1 on D+1. When the night can't absorb the freight it names tomorrow's salesfloor job groups as backup. Shift split and hours come from job codes (`lib/shifts.js`), not shift-start guessing. **Suggest plan** drafts the four blocks the store actually emails (Stock 2 / Overnight / Mod Team / Stock 1) using the line vocabulary and thresholds fitted from 12 real plans — see `dev/STOCKING_PLAN_FINDINGS.md` §6. A **?** button in the header opens a dialog explaining every hours calculation, with the per-department CPH table derived live from the collect that just ran (cases ÷ the hours CV charged for them) rather than hard-coded, since CPH is per store. |
